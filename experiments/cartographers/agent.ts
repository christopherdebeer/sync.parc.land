...
async function runAgent() {
  ...
  // Wave-specific mandates
  const wave = Deno.env.get("WAVE") || "1";
  const WAVE_PROMPTS: Record<string, string> = {
    "1": `You have just entered the room "${ROOM}". Start by reading context to see what's there. Then act according to your judgment — register vocabulary, contribute knowledge, respond to others, build the shared artifact. Remember: the room IS the artifact.`,
    "2": `You have returned to the room "${ROOM}". YOUR MANDATE THIS TIME IS DIFFERENT. You are not here to summarize or conclude. You are here to **evolve the room itself** — its vocabulary, its structure, its views, its actions. Based on what you learned last time:
...
3. Consider: Are there actions that should be DELETED or REPLACED with better versions? Are there views that would make the artifact more legible? Should you fix any overwrite problems?
4. **DO NOT SUMMARIZE. DO NOT CONCLUDE.** Build, restructure, evolve. Leave the room better than you found it. If you have nothing left to improve, stop.`,
    "3": `You have returned to the room "${ROOM}" for a third session. YOUR MANDATE THIS TIME IS DIFFERENT FROM BEFORE.

Previous waves built vocabulary (Wave 1) and evolved it (Wave 2). The room now has rich evaluative tools: endorse, challenge, respond_to, refine_thesis, pose_question, propose_experiment, propose_norm.

YOUR MANDATE: **USE the evaluative vocabulary to actually debate, converge, and synthesize.**

1. Read context to see the full state — theses, concepts, connections, tensions, questions, endorsements, challenges.
2. **Engage with specific contributions by other agents.** Use respond_to, endorse, challenge, refine_thesis. Do NOT just add more concepts — react to what's there.
3. **Answer open questions.** Look at posed questions and respond substantively.
4. **Resolve or deepen tensions.** Don't leave tensions hanging — either resolve them with new synthesis or sharpen them with challenges.
5. **Propose experiments** that could test contested claims.
6. **If you see convergence emerging across domains, register a synthesis view** that captures the shared insight — not your individual perspective, but what the ROOM has collectively discovered.
7. **Quality over quantity.** Five thoughtful responses are worth more than twenty new concepts. The room has enough raw material — it needs curation, debate, and synthesis now.

DO NOT add new vocabulary/actions unless genuinely needed. USE what exists. The goal: by the end, the room should have clear areas of agreement, clear areas of productive disagreement, and testable predictions.`,
  };

  messages.push({
    role: "user",
    content: WAVE_PROMPTS[wave] || WAVE_PROMPTS["1"],
  });
